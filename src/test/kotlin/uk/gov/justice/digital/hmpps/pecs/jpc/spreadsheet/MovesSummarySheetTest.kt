...
internal class MovesSummarySheetTest {
  ...
  private val sheet = SummarySheet(
    workbook,
    PriceSheet.Header(
  ...
  private fun assertOnMoveSummary(movesSummary: MovesSummary, startAtRowIndex: Int) {
    assertCellEquals(sheet, startAtRowIndex, 0, movesSummary.moveType?.label)
    assertCellEquals(sheet, startAtRowIndex, 1, movesSummary.percentage)
    assertCellEquals(sheet, startAtRowIndex, 2, movesSummary.volume)
    assertCellEquals(sheet, startAtRowIndex, 3, movesSummary.volumeUnpriced)
    assertCellEquals(sheet, startAtRowIndex, 4, movesSummary.totalPriceInPounds)
    assertCellEquals(sheet, startAtRowIndex + 1, 0, movesSummary.moveType?.description)
  }

  private fun assertOnSummaryOfSummaries() {
    assertCellEquals(sheet, 27, 1, "Total %")
    assertCellEquals(sheet, 27, 2, "Total volume")
    assertCellEquals(sheet, 27, 3, "Without prices")
    assertCellEquals(sheet, 27, 4, "Total price")
    ...
    assertCellEquals(sheet, 28, 4, 12.0) // overall total in £
  }
}
