...
internal class MoveModelPersisterTest {
    ...
    fun `Persist one move with one extant location and one journey`() {
        ...
        val fromLocation = fromPrisonLocationFactory()
        val toLocation = toCourtLocationFactory()
        ...
        val journey = journeyFactory().copy(id = UUID.randomUUID().toString(), billable = true)
        ...
        val completedMoveWithPricedBillableJourney = Report(
                ...
                journeysWithEvents = listOf(JourneyWithEvents(journey, listOf(
        ...
        val persister = MoveModelPersister(locationRepository, moveModelRepository)
