...
internal class AutomaticLocationMappingServiceTest {
  ...
  private val service: AutomaticLocationMappingService = AutomaticLocationMappingService(
    basmClientApiService,
    locationRepository,
    timeSource,
    auditService,
    monitoringService,
  ...
  fun `when there is a duplicate location it should be ignored and there should be basm and repository interactions only`() {
    ...
    val duplicateLocation =
      Location(duplicateBasmLocation.locationType, duplicateBasmLocation.agencyId, duplicateBasmLocation.name)
