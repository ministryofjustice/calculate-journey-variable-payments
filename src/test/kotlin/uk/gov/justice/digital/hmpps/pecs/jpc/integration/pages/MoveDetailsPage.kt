...
class MoveDetailsPage : ApplicationPage() {
  ...
  fun isAtPageFor(move: Move, expectedPrice: Money? = null) {
    ...
    assertThat(dateOfBirth.text()).isEqualTo(move.person?.dateOfBirth?.format(DateTimeFormatter.ofPattern("dd MM yyyy", Locale.UK)))
    logger.info("Checking gender")
    assertThat(gender.text()).isEqualTo(move.person?.gender)
    logger.info("Checking pick up date/time")
    assertThat(movePickupDate.text()).isEqualTo(move.pickUpDateTime?.format(DateTimeFormatter.ofPattern("dd MMM yyyy, HH:mm", Locale.UK)) ?: "Not known")
    logger.info("Checking drop off date/time")
    assertThat(moveDropoffDate.text()).isEqualTo(move.dropOffOrCancelledDateTime?.format(DateTimeFormatter.ofPattern("dd MMM yyyy, HH:mm", Locale.UK)))
