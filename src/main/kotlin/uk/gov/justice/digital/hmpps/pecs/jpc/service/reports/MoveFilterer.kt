...
object MoveFilterer {
   ...
   */
  fun isCancelledBillableMove(move: Move): Boolean = move.status == MoveStatus.cancelled &&
    move.reportFromLocationType == "prison" &&
    move.toNomisAgencyId != null &&
    move.reportToLocationType == "prison" &&
    move.events.hasEventType(EventType.MOVE_ACCEPT) &&
    // it was previously accepted
    move.events.hasEventType(EventType.MOVE_CANCEL) &&
    // it was cancelled
    (
  ...
  private fun List<Event>.hasMoveCancelEventDayBeforeMoveStartsAndAfter3pm(moveDate: LocalDate) = this.find {
   ...
   */
  fun isStandardMove(move: Move) = move.isCompleted() &&
    with(move.journeys) {
  ...
  private fun isCompleteBillableJourneyAndLocationsMatchMove(journey: Journey, move: Move) = journey.state == JourneyState.completed &&
    journey.billable &&
   ...
   */
  fun isLongHaulMove(move: Move) = move.isCompleted() &&
    (
   ...
   */
  fun isLockoutMove(move: Move) = move.isCompleted() &&
   ...
   */
  fun isRedirectionMove(move: Move) = move.isCompleted() &&
  ...
  private fun Event.isRedirectEventAfter(dateTime: LocalDateTime) = this.hasType(EventType.MOVE_REDIRECT) && this.occurredAt.isAfter(dateTime)
}
